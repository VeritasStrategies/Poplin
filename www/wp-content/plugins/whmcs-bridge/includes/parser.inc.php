...
function cc_whmcs_bridge_parser_ajax1($buffer, $page_to_include = '') {
    ...
    if (is_numeric($cache_setting) && $cache_setting > 0 && (
        ...
        cc_whmcs_log(0, 'Cacheable file - '.$url);

        if (file_exists($cache_dir) && is_writable($cache_dir)) {
            $prefix = md5($url);
            foreach (glob($cache_dir.$prefix.'_parsed*') as $filename) {
                if (strpos($filename, '_') !== false) {
                    $expires = explode('_', $filename);

                    if (time() > $expires[(count($expires)-1)]) {
                        unlink($filename);
                        cc_whmcs_log(0, 'Parse cache expired on - '.date('Y-m-d H:i:s', $expires[(count($expires)-1)]).' - '.$url);
                    } else {
                        cc_whmcs_log(0, 'Parse cache found for - '.$url.', fetching content from '.$filename);
    ...
    if ($process) {
        ...
        if (is_numeric($cache_setting) && $cache_setting > 0 && (
            ...
            if (file_exists($cache_dir) && is_writable($cache_dir)) {
                $filename = md5($url) . '_parsed_' . strtotime('+' . $cache_setting . ' minutes');
